; ModuleID = 'algorithms/02_c/techniques/sliding_window.c'
source_filename = "algorithms/02_c/techniques/sliding_window.c"
target datalayout = "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128"
target triple = "x86_64-pc-linux-gnu"

@str = private unnamed_addr constant [53 x i8] c"[C SlidingWindow] Fixed-size window max sum verified\00", align 1

; Function Attrs: nofree norecurse nosync nounwind sspstrong memory(argmem: read) uwtable
define dso_local i32 @max_subarray_sum(ptr noundef readonly captures(none) %0, i32 noundef %1, i32 noundef %2) local_unnamed_addr #0 {
  %4 = icmp sgt i32 %2, 0
  br i1 %4, label %5, label %29

5:                                                ; preds = %3
  %6 = zext nneg i32 %2 to i64
  %7 = icmp ult i32 %2, 8
  br i1 %7, label %26, label %8

8:                                                ; preds = %5
  %9 = and i64 %6, 2147483640
  br label %10

10:                                               ; preds = %10, %8
  %11 = phi i64 [ 0, %8 ], [ %20, %10 ]
  %12 = phi <4 x i32> [ zeroinitializer, %8 ], [ %18, %10 ]
  %13 = phi <4 x i32> [ zeroinitializer, %8 ], [ %19, %10 ]
  %14 = getelementptr inbounds nuw i32, ptr %0, i64 %11
  %15 = getelementptr inbounds nuw i8, ptr %14, i64 16
  %16 = load <4 x i32>, ptr %14, align 4, !tbaa !5
  %17 = load <4 x i32>, ptr %15, align 4, !tbaa !5
  %18 = add <4 x i32> %16, %12
  %19 = add <4 x i32> %17, %13
  %20 = add nuw i64 %11, 8
  %21 = icmp eq i64 %20, %9
  br i1 %21, label %22, label %10, !llvm.loop !9

22:                                               ; preds = %10
  %23 = add <4 x i32> %19, %18
  %24 = tail call i32 @llvm.vector.reduce.add.v4i32(<4 x i32> %23)
  %25 = icmp eq i64 %9, %6
  br i1 %25, label %29, label %26

26:                                               ; preds = %5, %22
  %27 = phi i64 [ 0, %5 ], [ %9, %22 ]
  %28 = phi i32 [ 0, %5 ], [ %24, %22 ]
  br label %53

29:                                               ; preds = %53, %22, %3
  %30 = phi i32 [ 0, %3 ], [ %24, %22 ], [ %58, %53 ]
  %31 = icmp slt i32 %2, %1
  br i1 %31, label %32, label %61

32:                                               ; preds = %29
  %33 = sext i32 %2 to i64
  %34 = sext i32 %1 to i64
  %35 = sub nsw i64 %34, %33
  %36 = and i64 %35, 1
  %37 = icmp eq i64 %36, 0
  br i1 %37, label %46, label %38

38:                                               ; preds = %32
  %39 = getelementptr inbounds i32, ptr %0, i64 %33
  %40 = load i32, ptr %39, align 4, !tbaa !5
  %41 = load i32, ptr %0, align 4, !tbaa !5
  %42 = sub nsw i32 %40, %41
  %43 = add nsw i32 %42, %30
  %44 = tail call i32 @llvm.smax.i32(i32 %43, i32 %30)
  %45 = add nsw i64 %33, 1
  br label %46

46:                                               ; preds = %38, %32
  %47 = phi i32 [ poison, %32 ], [ %44, %38 ]
  %48 = phi i64 [ %33, %32 ], [ %45, %38 ]
  %49 = phi i32 [ %30, %32 ], [ %44, %38 ]
  %50 = phi i32 [ %30, %32 ], [ %43, %38 ]
  %51 = add nsw i64 %34, -1
  %52 = icmp eq i64 %51, %33
  br i1 %52, label %61, label %63

53:                                               ; preds = %26, %53
  %54 = phi i64 [ %59, %53 ], [ %27, %26 ]
  %55 = phi i32 [ %58, %53 ], [ %28, %26 ]
  %56 = getelementptr inbounds nuw i32, ptr %0, i64 %54
  %57 = load i32, ptr %56, align 4, !tbaa !5
  %58 = add nsw i32 %57, %55
  %59 = add nuw nsw i64 %54, 1
  %60 = icmp eq i64 %59, %6
  br i1 %60, label %29, label %53, !llvm.loop !13

61:                                               ; preds = %46, %63, %29
  %62 = phi i32 [ %30, %29 ], [ %47, %46 ], [ %83, %63 ]
  ret i32 %62

63:                                               ; preds = %46, %63
  %64 = phi i64 [ %84, %63 ], [ %48, %46 ]
  %65 = phi i32 [ %83, %63 ], [ %49, %46 ]
  %66 = phi i32 [ %82, %63 ], [ %50, %46 ]
  %67 = getelementptr inbounds i32, ptr %0, i64 %64
  %68 = load i32, ptr %67, align 4, !tbaa !5
  %69 = sub nsw i64 %64, %33
  %70 = getelementptr inbounds i32, ptr %0, i64 %69
  %71 = load i32, ptr %70, align 4, !tbaa !5
  %72 = sub nsw i32 %68, %71
  %73 = add nsw i32 %72, %66
  %74 = tail call i32 @llvm.smax.i32(i32 %73, i32 %65)
  %75 = add nsw i64 %64, 1
  %76 = getelementptr inbounds i32, ptr %0, i64 %75
  %77 = load i32, ptr %76, align 4, !tbaa !5
  %78 = sub nsw i64 %75, %33
  %79 = getelementptr inbounds i32, ptr %0, i64 %78
  %80 = load i32, ptr %79, align 4, !tbaa !5
  %81 = sub nsw i32 %77, %80
  %82 = add nsw i32 %81, %73
  %83 = tail call i32 @llvm.smax.i32(i32 %82, i32 %74)
  %84 = add nsw i64 %64, 2
  %85 = icmp eq i64 %84, %34
  br i1 %85, label %61, label %63, !llvm.loop !14
}

; Function Attrs: nofree nounwind sspstrong uwtable
define dso_local noundef range(i32 0, 2) i32 @main() local_unnamed_addr #1 {
  %1 = tail call i32 @puts(ptr nonnull dereferenceable(1) @str)
  ret i32 0
}

; Function Attrs: nofree nounwind
declare noundef i32 @puts(ptr noundef readonly captures(none)) local_unnamed_addr #2

; Function Attrs: nocallback nocreateundeforpoison nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.smax.i32(i32, i32) #3

; Function Attrs: nocallback nofree nosync nounwind speculatable willreturn memory(none)
declare i32 @llvm.vector.reduce.add.v4i32(<4 x i32>) #4

attributes #0 = { nofree norecurse nosync nounwind sspstrong memory(argmem: read) uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #1 = { nofree nounwind sspstrong uwtable "min-legal-vector-width"="0" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="x86-64" "target-features"="+cmov,+cx8,+fxsr,+mmx,+sse,+sse2,+x87" "tune-cpu"="generic" }
attributes #2 = { nofree nounwind }
attributes #3 = { nocallback nocreateundeforpoison nofree nosync nounwind speculatable willreturn memory(none) }
attributes #4 = { nocallback nofree nosync nounwind speculatable willreturn memory(none) }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}
!llvm.errno.tbaa = !{!5}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"PIE Level", i32 2}
!3 = !{i32 7, !"uwtable", i32 2}
!4 = !{!"clang version 22.1.8"}
!5 = !{!6, !6, i64 0}
!6 = !{!"int", !7, i64 0}
!7 = !{!"omnipotent char", !8, i64 0}
!8 = !{!"Simple C/C++ TBAA"}
!9 = distinct !{!9, !10, !11, !12}
!10 = !{!"llvm.loop.mustprogress"}
!11 = !{!"llvm.loop.isvectorized", i32 1}
!12 = !{!"llvm.loop.unroll.runtime.disable"}
!13 = distinct !{!13, !10, !12, !11}
!14 = distinct !{!14, !10}
