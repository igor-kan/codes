...
function levenbergMarquardt(model, jacobian, xs, ys, initial, damping = 1e-3, maxIterations = 200) {
...
if (require.main === module) {
  ...
  const model = (x, p) => p[0] + p[1] * x + p[2] * x * x;
  const jacobian = (x) => [1, x, x * x];
  const fitted = levenbergMarquardt(model, jacobian, xs, ys, [0, 0, 0]);
  if (Math.abs(fitted[0] - 1) > 1e-6 || Math.abs(fitted[1] - 2) > 1e-6 || Math.abs(fitted[2] - 3) > 1e-6) {
    throw new Error("levenberg marquardt failed");
  }
  console.log("[JavaScript Levenberg-Marquardt] curve fit verified");
}
