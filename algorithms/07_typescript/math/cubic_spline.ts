...
interface Spline { b: number[]; c: number[]; d: number[] }

function naturalCubicSpline(xs: number[], ys: number[]): Spline {
  const n = xs.length;
...
function splineEvaluate(xs: number[], ys: number[], spline: Spline, x: number): number {
  const n = xs.length;
  let segment = n - 2;
...
console.log("cubic spline verified");
