function nelderMead(f, start, step = 0.5, tolerance = 1e-10, maxIterations = 2000) {
  ...
  for (let iteration = 0; iteration < maxIterations; iteration += 1) {
    ...
    if (f(simplex[0]) <= f(reflected) && f(reflected) < f(simplex[n - 1])) {
      ...
      if (f(contracted) < f(worst)) simplex[n] = contracted;
        ...
        for (let j = 1; j <= n; j += 1) {
          simplex[j] = simplex[j].map((value, i) => (simplex[0][i] + value) / 2);
        }
      }
    }
  }
...
if (require.main === module) {
  ...
  console.log("[JavaScript Nelder-Mead] minimum verified");
}
